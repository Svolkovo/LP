...
document.addEventListener("DOMContentLoaded", () => {
  var preloader = document.querySelector(".preloader-new");
  //if(preloader === null) return;

  console.log(preloader);
  var firstLine = document.querySelector(".lineone-container");
  var secondLine = document.querySelector(".linetwo-container");
  var firstSvg = firstLine.querySelector("svg");
  var secondSvg = secondLine.querySelector("svg");
  var greenline = document.querySelector(".linegreen");
  ...
  var greenlineOffsetTop = greenline.getBoundingClientRect().top;
  var preloaderOffsetTop = preloader.getBoundingClientRect().top;

  // Вычисляем расстояние, чтобы поднять `greenline` до верхней границы `preloader`
  var distanceToTop = greenlineOffsetTop - preloaderOffsetTop;
  tl.to(greenline, {
    y: "-".concat(distanceToTop, "px"),
    delay: 0.6,
    duration: 1.1
  });
  tl.set(greenline, {
  ...
  tl.to(greenline, {
    x: "-70vw",
    width: "165vw",
    // расширяем до 120% от ширины экрана
    duration: 2,
    onComplete: () => {
      gsap.set(greenline, {
        "position": 'fixed',
